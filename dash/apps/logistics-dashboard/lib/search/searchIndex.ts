import type { PoiLocation } from '../map/poiTypes';
...
export type SearchItem = {
  ...
  /** used for matching */
  tokens: string[];
  /** payload for navigation/selection */
...
export function buildSearchIndex(args: {
  worklistRows?: ReadonlyArray<Record<string, unknown>>;
  ...
  for (const r of rows) {
    const hvdc = (r['hvdc_code'] ?? r['ref_no'] ?? r['refNo'] ?? r['ref'] ?? r['code']) as
      | string
      | undefined;
    const caseNo = (r['case_no'] ?? r['caseNo'] ?? r['case_id'] ?? r['caseId']) as string | undefined;
    const name = (r['name'] ?? r['project_name'] ?? r['projectName']) as string | undefined;
    const loc = (r['final_location'] ?? r['location_code'] ?? r['locationCode'] ?? r['location']) as
      | string
      | undefined;

    const tokens = uniqTokens([hvdc ?? '', caseNo ?? '', name ?? '', loc ?? '']);
...
export function searchIndex(args: {
  ...
  for (const it of args.items) {
    ...
    // Small tie-breaker: prefer shipments > cases > locations
