...
- (추가) shipments_case.csv / events_case.csv / events_case_debug.csv (대시보드 alias)
- report.json / report.md
- hvdc_ops_data.ttl (옵션: --export-ttl)
...
def _extract_ids(record: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if case_raw in (None, ""):
        # Case No.가 없으면 synthetic TEXT 키 생성 (ALL SSOT 유지용)
        row_no = (
            record.get("no.")
            or record.get("No.")
            or record.get("NO")
        ...
        row_no_str = str(row_no).strip() if row_no not in (None, "") else ""
        case_key = f"NA-{row_no_str}" if row_no_str else "NA-UNKNOWN"
    else:
        case_key = str(case_raw).strip()
    return hvdc_code, case_key
...
def left_merge_all_wh(
    ...
    """
    wh_index: Dict[Tuple[str, str], Dict[str, Any]] = {}
    stats = {
...
def build_flows_option_c(
    ...
    for i, row in df_calc.iterrows():
        # dict로 변환 후 _extract_ids 재사용하여 TEXT case_key 확보
        rec = row.to_dict()
        try:
            hvdc_code, case_key = _extract_ids(rec)
        except Exception:
            continue
...
def run_etl(
    ...
    ontology_ttl: Optional[Path] = None,
    shapes_ttl: Optional[Path] = None,
    ttl_name: str = "hvdc_ops_data.ttl",
) -> None:
    ...
    # Dashboard-friendly aliases (same content)
    try:
        (output_dir / "shipments_case.csv").write_text((output_dir / "shipments.csv").read_text(encoding="utf-8"), encoding="utf-8")
        (output_dir / "events_case.csv").write_text((output_dir / "events.csv").read_text(encoding="utf-8"), encoding="utf-8")
        (output_dir / "events_case_debug.csv").write_text((output_dir / "events_debug.csv").read_text(encoding="utf-8"), encoding="utf-8")
    except Exception:
        pass
    ...
    if export_ttl:
        export_ops_ttl_option_c(
            out_ttl=output_dir / ttl_name,
            base_iri=base_iri,
            shipments=shipments,
            cases=cases,
            flows=flows,
            locations=locations,
            events=events,
            events_dbg=events_dbg,
            ontology_ttl=ontology_ttl,
            shapes_ttl=shapes_ttl,
        )
# ==========================
# OPS TTL Export (embedded)
# - instance IRIs follow: {base_iri}/Shipment/{hvdc_code}, {base_iri}/Case/{hvdc_code}/{case_no}, ...
# - predicates follow hvdc_ops_ontology.ttl
# ==========================
...
def _hash20(*vals: str) -> str:
    h = hashlib.sha256("||".join([str(v) for v in vals]).encode("utf-8")).hexdigest()
    return h[:20]

def _copy_if_exists(src: Optional[Path], dst_dir: Path) -> None:
    if not src:
        return
    try:
        if src.exists():
            (dst_dir / src.name).write_text(src.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    except Exception:
        return

def _autofind_schema_shapes(script_dir: Path) -> Tuple[Optional[Path], Optional[Path]]:
    cand1 = script_dir / "hvdc_ops_ontology.ttl"
    cand2 = script_dir / "hvdc_ops_shapes.ttl"
    if cand1.exists() and cand2.exists():
        return cand1, cand2
    # also try cwd
    cwd1 = Path.cwd() / "hvdc_ops_ontology.ttl"
    cwd2 = Path.cwd() / "hvdc_ops_shapes.ttl"
    if cwd1.exists() and cwd2.exists():
        return cwd1, cwd2
    return None, None

def export_ops_ttl_option_c(
    out_ttl: Path,
    base_iri: str,
    ...
    """
    Option-C instance TTL writer aligned to hvdc_ops_ontology.ttl
    Writes:
      - Location instances
      - Shipment instances
      - Case instances + hasCase links
      - Flow instances + hasFlow links
      - CaseEvent instances + hasEvent links
    """
    out_ttl.parent.mkdir(parents=True, exist_ok=True)
    hvdc_ns = base_iri.rstrip("/") + "#"

    # reverse map for location_id -> location_code
    locid_to_code = {v.location_id: v.location_code for v in locations.values()}

    lines: List[str] = []
    lines.append(f"@prefix hvdc: <{hvdc_ns}> .")
    lines.append("@prefix xsd:  <http://www.w3.org/2001/XMLSchema#> .")
    lines.append("")
    lines.append("# Instance data (Option-C Case layer)")
    lines.append("")

    # Locations
    for code in sorted(locations.keys()):
        l = locations[code]
        loc_iri = f"<{_iri(base_iri, 'Location', l.location_code)}>"
        lines.append(f"{loc_iri} a hvdc:Location ;")
        lines.append(f'  hvdc:locationId "{int(l.location_id)}"^^xsd:integer ;')
        lines.append(f'  hvdc:locationCode "{_ttl_escape(l.location_code)}" ;')
        lines.append(f'  hvdc:locationName "{_ttl_escape(l.name)}" ;')
        lines.append(f'  hvdc:locationCategory "{_ttl_escape(l.category)}" ;')
        if l.hvdc_node:
            lines.append(f'  hvdc:hvdcNode "{_ttl_escape(l.hvdc_node)}" ;')
        lines.append(f'  hvdc:isMosb "{str(l.is_mosb).lower()}"^^xsd:boolean ;')
        lines.append(f'  hvdc:isSite "{str(l.is_site).lower()}"^^xsd:boolean ;')
        lines.append(f'  hvdc:isPort "{str(l.is_port).lower()}"^^xsd:boolean ;')
        lines.append(f'  hvdc:active "{str(l.active).lower()}"^^xsd:boolean .')
        lines.append("")

    # Shipments
    for s in sorted(shipments, key=lambda x: x.hvdc_code):
        ship_iri = f"<{_iri(base_iri, 'Shipment', s.hvdc_code)}>"
        lines.append(f"{ship_iri} a hvdc:Shipment ;")
        lines.append(f'  hvdc:hvdcCode "{_ttl_escape(s.hvdc_code)}" ;')
        if s.vendor:
            lines.append(f'  hvdc:vendor "{_ttl_escape(s.vendor)}" ;')
        if s.coe:
            lines.append(f'  hvdc:rawJson """{{"COE":"{_ttl_escape(s.coe)}"}}""" ;')
        if s.pol:
            lines.append(f'  hvdc:pol "{_ttl_escape(s.pol)}" ;')
        if s.pod:
            lines.append(f'  hvdc:pod "{_ttl_escape(s.pod)}" ;')
        if s.vessel:
            lines.append(f'  hvdc:vessel "{_ttl_escape(s.vessel)}" ;')
        if s.hs_code:
            lines.append(f'  hvdc:rawJson """{{"HS Code":"{_ttl_escape(s.hs_code)}"}}""" ;')
        if s.currency:
            lines.append(f'  hvdc:currency "{_ttl_escape(s.currency)}" ;')
        if s.shipment_invoice_no:
            lines.append(f'  hvdc:rawJson """{{"Shipment Invoice No.":"{_ttl_escape(s.shipment_invoice_no)}"}}""" ;')
        if s.price is not None:
            lines.append(f'  hvdc:rawJson """{{"Price":{s.price}}}""" ;')
        # terminate
        lines[-1] = lines[-1].rstrip(" ;") + " ."
        lines.append("")

    # Cases + link to shipment
    for c in cases:
        case_iri = f"<{_iri(base_iri, 'Case', c.hvdc_code, c.case_no)}>"
        ship_iri = f"<{_iri(base_iri, 'Shipment', c.hvdc_code)}>"
        lines.append(f"{case_iri} a hvdc:Case ;")
        lines.append(f'  hvdc:hvdcCode "{_ttl_escape(c.hvdc_code)}" ;')
        lines.append(f'  hvdc:caseNo "{_ttl_escape(c.case_no)}" ;')
        lines.append(f"  hvdc:belongsToShipment {ship_iri} ;")
        if c.site_code:
            lines.append(f'  hvdc:siteCode "{_ttl_escape(c.site_code)}" ;')
        if c.eq_no:
            lines.append(f'  hvdc:eqNo "{_ttl_escape(c.eq_no)}" ;')
        if c.pkg is not None:
            lines.append(f'  hvdc:pkg "{int(c.pkg)}"^^xsd:integer ;')
        if c.description:
            lines.append(f'  hvdc:description "{_ttl_escape(c.description)}" ;')
        if c.final_location:
            lines.append(f'  hvdc:finalLocation "{_ttl_escape(c.final_location)}" ;')
        if c.storage:
            lines.append(f'  hvdc:storage "{_ttl_escape(c.storage)}" ;')
        for val, pred in [(c.l_cm,"hvdc:lCm"),(c.w_cm,"hvdc:wCm"),(c.h_cm,"hvdc:hCm"),(c.cbm,"hvdc:cbm"),(c.nw_kg,"hvdc:nwKg"),(c.gw_kg,"hvdc:gwKg"),(c.sqm,"hvdc:sqm")]:
            if val is not None:
                lines.append(f'  {pred} "{val}"^^xsd:decimal ;')
        if c.vendor:
            lines.append(f'  hvdc:vendor "{_ttl_escape(c.vendor)}" ;')
        lines[-1] = lines[-1].rstrip(" ;") + " ."
        lines.append("")
        # shipment hasCase
        lines.append(f"{ship_iri} hvdc:hasCase {case_iri} .")
        lines.append("")

    # Flows + link to case
    flow_index = {(f.hvdc_code, f.case_no): f for f in flows}
    for c in cases:
        f = flow_index.get((c.hvdc_code, c.case_no))
        if not f:
            continue
        case_iri = f"<{_iri(base_iri, 'Case', c.hvdc_code, c.case_no)}>"
        flow_iri = f"<{_iri(base_iri, 'Flow', c.hvdc_code, c.case_no)}>"
        lines.append(f"{flow_iri} a hvdc:Flow ;")
        lines.append(f'  hvdc:hvdcCode "{_ttl_escape(f.hvdc_code)}" ;')
        lines.append(f'  hvdc:caseNo "{_ttl_escape(f.case_no)}" ;')
        lines.append(f'  hvdc:flowCode "{int(f.flow_code)}"^^xsd:integer ;')
        if f.flow_code_original is not None:
            lines.append(f'  hvdc:flowCodeOriginal "{int(f.flow_code_original)}"^^xsd:integer ;')
        if f.flow_code_derived is not None:
            lines.append(f'  hvdc:flowCodeDerived "{int(f.flow_code_derived)}"^^xsd:integer ;')
        if f.override_reason:
            lines.append(f'  hvdc:overrideReason "{_ttl_escape(f.override_reason)}" ;')
        if f.warehouse_count is not None:
            lines.append(f'  hvdc:warehouseCount "{int(f.warehouse_count)}"^^xsd:integer ;')
        lines.append(f'  hvdc:hasMosbLeg "{str(f.has_mosb_leg).lower()}"^^xsd:boolean ;')
        lines.append(f'  hvdc:hasSiteArrival "{str(f.has_site_arrival).lower()}"^^xsd:boolean ;')
        if f.customs_code:
            lines.append(f'  hvdc:customsCode "{_ttl_escape(f.customs_code)}" ;')
        if f.customs_start_iso:
            lines.append(f'  hvdc:customsStart "{_ttl_escape(f.customs_start_iso)}"^^xsd:dateTime ;')
        if f.customs_end_iso:
            lines.append(f'  hvdc:customsEnd "{_ttl_escape(f.customs_end_iso)}"^^xsd:dateTime ;')
        if f.last_status:
            lines.append(f'  hvdc:lastStatus "{_ttl_escape(f.last_status)}" ;')
        lines.append(f'  hvdc:requiresReview "{str(f.requires_review).lower()}"^^xsd:boolean .')
        lines.append("")
        lines.append(f"{case_iri} hvdc:hasFlow {flow_iri} .")
        lines.append("")

    # Events (CaseEvent)
    for e in events:
        loc_code = locid_to_code.get(int(e.location_id))
        if not loc_code:
            continue
        # deterministic id from natural key
        hid = _hash20(e.hvdc_code, str(e.case_no), e.event_type, e.event_time_iso, loc_code, e.source_field, e.source_system)
        ev_iri = f"<{_iri(base_iri, 'CaseEvent', e.hvdc_code, str(e.case_no), hid)}>"
        case_iri = f"<{_iri(base_iri, 'Case', e.hvdc_code, str(e.case_no))}>"
        loc_iri = f"<{_iri(base_iri, 'Location', loc_code)}>"
        lines.append(f"{ev_iri} a hvdc:CaseEvent ;")
        lines.append(f'  hvdc:eventId "{hid}" ;')
        lines.append(f'  hvdc:eventType "{_ttl_escape(e.event_type)}" ;')
        lines.append(f'  hvdc:eventTime "{_ttl_escape(e.event_time_iso)}"^^xsd:dateTime ;')
        lines.append(f'  hvdc:sourceSystem "{_ttl_escape(e.source_system)}" ;')
        lines.append(f'  hvdc:sourceField "{_ttl_escape(e.source_field)}" ;')
        if e.raw_epoch_ms is not None:
            lines.append(f'  hvdc:rawEpochMs "{int(e.raw_epoch_ms)}"^^xsd:integer ;')
        lines.append(f"  hvdc:atLocation {loc_iri} ;")
        lines.append(f"  hvdc:forCase {case_iri} .")
        lines.append("")
        lines.append(f"{case_iri} hvdc:hasEvent {ev_iri} .")
        lines.append("")

    out_ttl.write_text("\n".join(lines), encoding="utf-8")

    # copy schema/shapes if provided (or auto found)
    if ontology_ttl is None or shapes_ttl is None:
        auto_o, auto_s = _autofind_schema_shapes(Path(__file__).parent)
        ontology_ttl = ontology_ttl or auto_o
        shapes_ttl = shapes_ttl or auto_s
    _copy_if_exists(ontology_ttl, out_ttl.parent)
    _copy_if_exists(shapes_ttl, out_ttl.parent)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--output-dir", default="output_supabase_optionC", help="Output directory"
    )
    p.add_argument(
        "--export-ttl",
        action="store_true",
        help="Also export hvdc_ops_data.ttl (instances, aligned to hvdc_ops_ontology.ttl)",
    )
    p.add_argument("--base-iri", default="https://example.com/hvdc", help="OPS TTL instance base IRI (no #).")
    p.add_argument("--ontology-ttl", default="", help="Path to hvdc_ops_ontology.ttl to copy next to output TTL (optional).")
    p.add_argument("--shapes-ttl", default="", help="Path to hvdc_ops_shapes.ttl to copy next to output TTL (optional).")
    p.add_argument("--ttl-name", default="hvdc_ops_data.ttl", help="TTL filename under output-dir when --export-ttl.")
    return p
...
def main() -> None:
    ...
    ontology_ttl = Path(args.ontology_ttl).expanduser().resolve() if args.ontology_ttl else None
    shapes_ttl = Path(args.shapes_ttl).expanduser().resolve() if args.shapes_ttl else None
    ...
    run_etl(
        ...
        customs_path,
        out_dir,
        export_ttl=bool(args.export_ttl),
        base_iri=str(args.base_iri),
        ontology_ttl=ontology_ttl,
        shapes_ttl=shapes_ttl,
        ttl_name=str(args.ttl_name),
    )
