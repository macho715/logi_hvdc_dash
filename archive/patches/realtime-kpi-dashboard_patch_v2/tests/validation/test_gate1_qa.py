...
@pytest.mark.validation
def test_gate1_coverage_sanity() -> None:
    """Coverage sanity checks.

    docs/DATA_LOADING_PLAN.md includes a "Coverage" section for Gate 1 QA.
    It does not specify a strict equality condition (events may not exist for
    every hvdc_code), so we assert basic relationships and non-emptiness.
    """

    _skip_if_no_db()

    shipments_count = int(fetch_one("SELECT COUNT(DISTINCT hvdc_code)::bigint FROM status.shipments_status;")[0])
    events_shipments_count = int(fetch_one("SELECT COUNT(DISTINCT hvdc_code)::bigint FROM status.events_status;")[0])

    cases_count = int(fetch_one('SELECT COUNT(*)::bigint FROM "case".cases;')[0])
    unique_hvdc_codes = int(fetch_one('SELECT COUNT(DISTINCT hvdc_code)::bigint FROM "case".cases;')[0])

    assert shipments_count > 0, "status.shipments_status has no hvdc_code rows"
    assert events_shipments_count >= 0
    assert events_shipments_count <= shipments_count, "events_status hvdc_code set should be a subset of shipments_status"

    assert cases_count > 0, "case.cases is empty"
    assert unique_hvdc_codes > 0
    assert unique_hvdc_codes <= cases_count
