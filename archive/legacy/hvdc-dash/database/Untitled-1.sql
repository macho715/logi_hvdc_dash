...
CREATE TABLE warehouse_inventory (
    ...
    -- 프로젝트별 수량 (2차 분류)
    project_shu2 INTEGER DEFAULT 0,
    project_mir3 INTEGER DEFAULT 0,
    project_das4 INTEGER DEFAULT 0,
    project_agi5 INTEGER DEFAULT 0,
    
    -- DSV 창고
    dsv_indoor NUMERIC(10,2),
    dsv_outdoor NUMERIC(10,2),
    dsv_mzd NUMERIC(10,2),
    
    -- JDN 창고
    jdn_mzd NUMERIC(10,2),
    jdn_waterfront NUMERIC(10,2),
    
    -- 기타 창고
    mosb NUMERIC(10,2),
    aaa_storage NUMERIC(10,2),
    zener_wh NUMERIC(10,2),
    hauler_dg_storage NUMERIC(10,2),
    vijay_tanks DATE, -- 특수: 날짜 필드로 보임
    
    -- 총 재고량 계산
    total_inventory NUMERIC(12,2) GENERATED ALWAYS AS (
        COALESCE(dsv_indoor, 0) + COALESCE(dsv_outdoor, 0) +
        COALESCE(dsv_mzd, 0) + COALESCE(jdn_mzd, 0) +
        COALESCE(jdn_waterfront, 0) + COALESCE(mosb, 0) +
        COALESCE(aaa_storage, 0) + COALESCE(zener_wh, 0) +
        COALESCE(hauler_dg_storage, 0)
    ) STORED,
